        ...
        <div class="container">
            ...
            <div class="mx-auto shadow p-5 rounded">
                ...
                <div class="auth-form-container text-left mx-auto">
                    <form class="auth-form login-form" method="POST" action="{{ route('updateStacks', Auth::user()->id) }}">
                                      ...
                                      <div class="card shadow-sm">
                                        <div class="card-body">
                                          ...
                                          <h5 class="card-title mb-3">
                                            ...
                                            @foreach($user->stacks as $stark)

                                              @if($stack->id == $stark->id)

                                                <span class=""><input id="{{ $stackId }}" type="checkbox" name="stacks[]" value="{{ $stack->id }}" onclick="ShowHideRole({{ $stackId }}, {{ $levelId }}, {{ $yearId }})" checked> {{ $stack->name }}</span>
                                                </h5>

                                                <div class="card-text">

                                                   <label class="form-label">Level Of Expertise</label>
                                                    <select id="{{ $levelId }}" name="levels[]" class="form-control custom-select" style="padding-top: 10px; padding-bottom: 10px">
                                                      @foreach($levels as $level)

                                                        @if($stark->pivot->level == $level->id)
                                                    ...
                                                    <label class="form-label">Years Of Experience</label> <br>
                                                        <input style="width: 100%" id="{{ $yearId }}" type="number" name="years[]" min="1" max="50" value="{{ $stark->pivot->years }}">
